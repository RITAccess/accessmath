...
/*
 ...
 Determines what the current orientation of the device is and what the
 appropriate placement for the UIButtons will be
 Adds the left swipe functionality to display the background options for the
 papers/nodes.
 Adds the zoom in/zoom out functionality.
 Adds the panning ability.
 Checks to see if the device orientation is changed at any point to determine
 the appropriate view change via NSNotificationCenter.
 */
- (void)didMoveToView:(SKView *)view {
  ...
  [newPaper addTarget:self
                action:@selector(newPaper)
      forControlEvents:UIControlEventTouchUpInside];
  ...
  leftSwipe =
      [[UISwipeGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(leftFlip:)];
  [leftSwipe setDirection:UISwipeGestureRecognizerDirectionLeft];
  [leftSwipe setNumberOfTouchesRequired:2];
  [view addGestureRecognizer:leftSwipe];

  zoomIn = [[UIPinchGestureRecognizer alloc]
      initWithTarget:self
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)panGesture {
  CGPoint translation =
      [panGesture translationInView:panGesture.view.superview];
  _originalCenter = panRecognizer.view.center;
  if (UIGestureRecognizerStateBegan == panGesture.state ||
      UIGestureRecognizerStateChanged == panGesture.state) {
    panGesture.view.center =
        CGPointMake(panGesture.view.center.x + translation.x,
                    panGesture.view.center.y + translation.y);
    // Reset translation, so we can get translation delta's (i.e. change in
    // translation)
    ...
    float slideFactor =
        0.1 * slideMult /
        4;  // Increase for more of a slide (original doesn't have a divider)
    CGPoint finalPoint =
        CGPointMake(panGesture.view.center.x + (velocity.x * slideFactor),
                    panGesture.view.center.y + (velocity.y * slideFactor));
    finalPoint.x = MIN(MAX(finalPoint.x, 70), self.view.bounds.size.width - 70);
    finalPoint.y =
        MIN(MAX(finalPoint.y, 100), self.view.bounds.size.height - 100);
...
- (void)handlePinchGesture:(UIPinchGestureRecognizer *)pinchGesture {
  if (UIGestureRecognizerStateBegan == pinchGesture.state ||
    ...
    // Use the x or y scale, they should be the same for typical zooming
    // (non-skewing)
    float currentScale = [[pinchGesture.view.layer
    ...
    // can multiply a speed factor and then translate back to "zoomSpace" around
    // 1
    ...
    CGAffineTransform zoomTransform = CGAffineTransformScale(
        pinchGesture.view.transform, deltaScale, deltaScale);
...
/*
 Checks to see if the change background button has been clicked, and opens the
 menu to choose the
...
- (void)leftFlip:(id)sender {
  ...
  BackgroundImages *backy =
      [[BackgroundImages alloc] initWithSize:CGSizeMake(1024, 768)];
...
- (SKTexture *)originalTexture {
  ...
  pap.position =
      CGPointMake(CGRectGetMidX(outline.frame), CGRectGetMidY(outline.frame));
...
- (SKTexture *)makeNotecardWithSavedImage {
  ...
  for (BackgroundTile *image in [images createTiles]) {
    if ([[saveData sharedData]
                .colorName isEqualToString:image.backgroundTileName]) {
      if (image.isColor) {
        node = [SKSpriteNode spriteNodeWithColor:image.colorName
...
- (void)newPaper {
  ...
  if ([saveData sharedData].current != nil) {
    newPap =
        [[SKSpriteNode alloc] initWithTexture:[saveData sharedData].current];
  } else {
  ...
  /*
   Creating more nodes that have their physicsBodies instantiated and can adapt
   to changes in texture like those in
...
- (void)addPositionConstraints:(SKSpriteNode *)notecard {
  ...
  SKConstraint *positionConstraint = [SKConstraint
      positionX:[SKRange rangeWithLowerLimit:150 upperLimit:(scr.width - 150)]
              Y:[SKRange rangeWithLowerLimit:200
                                  upperLimit:(scr.height - 100)]];
...
- (void)addDeleteButton:(SKSpriteNode *)sprite {
  ...
  SKSpriteNode *delete =
      [[SKSpriteNode alloc] initWithColor:[SKColor redColor]
...
- (void)createScene {
  ...
  if ([[saveData sharedData].colorName length] == 0) {
    // for the following nodes, the ability to return to default needs to be
    // instantiated
    ...
    newNode = [[SKSpriteNode alloc]
        initWithTexture:[self makeNotecardWithSavedImage]];
    newNode2 = [[SKSpriteNode alloc]
        initWithTexture:[self makeNotecardWithSavedImage]];
    newNode3 = [[SKSpriteNode alloc]
        initWithTexture:[self makeNotecardWithSavedImage]];
  }
...
- (void)didBeginContact:(SKPhysicsContact *)contact {
  ...
  if ((firstBody.categoryBitMask == (outline2Category | outline3Category)) ||
      (secondBody.categoryBitMask == (outline2Category | outline3Category))) {
  }
}

// make physics body for specific notecard
- (void)makePhysicsBody:(SKSpriteNode *)notecard {
  notecard.physicsBody =
...
// in this section, the nodes' backgrounds will be able to by customized by the
// students using Access Math
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  /*
   The following section is divided into cycling through nodes with different
   names.
   Each has different properties.
   The first if checks to see if the node being clicked is the SKSpriteNode for
   the paper itself, and thus checks to see if
  ...
  if (checkNode && ([checkNode.name hasPrefix:@"newNode"])) {
    if ([touch tapCount] == 2) {
      ...
      if ([checkNode.name isEqualToString:@"newNode"] ||
          [checkNode.name isEqualToString:@"newNode2"] ||
          [checkNode.name isEqualToString:@"newNode3"]) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"gotoNotes"
                                                            object:nil];
      } else {
        [[NSNotificationCenter defaultCenter]
            postNotificationName:@"gotoNewNotes"
                          object:nil];
      }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (_activeDragNode != nil) {
    ...
    CGPoint newLoc =
        CGPointMake(_activeDragNode.position.x + (scenePosition.x - lastPos.x),
...
/*
 Detects when the "touches" has ended and re-instantiates the swipe and pan
 gestures.
 */
