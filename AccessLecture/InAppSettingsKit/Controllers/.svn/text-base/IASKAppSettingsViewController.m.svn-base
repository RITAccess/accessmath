...
/**
 Cancels any changes made since they went to the settings page
 */
- (IBAction)cancelChanges {
    
    [_defaults setValuesForKeysWithDictionary:oldDefaults];
    UIAlertView* alert = [[UILargeAlertView alloc]
                          initWithText:NSLocalizedString(@"Changes Canceled", nil)
                          fontSize:54];
    [alert show];
...
-(void)settingsChange {
    ...
    // Toolbar colors
    float toolRed = [_defaults floatForKey:@"tbRed"];
    float toolGreen = [_defaults floatForKey:@"tbGreen"];
    float toolBlue = [_defaults floatForKey:@"tbBlue"];
    [_toolbar setBackgroundColor:[UIColor colorWithRed:toolRed green:toolGreen blue:toolBlue alpha:1.0]];
    
    // Text colors
    float textRed = [_defaults floatForKey:@"textRed"];
    float textGreen = [_defaults floatForKey:@"textGreen"];
    float textBlue = [_defaults floatForKey:@"textBlue"];
    _toolbarLabel.textColor = [UIColor colorWithRed:textRed green:textGreen blue:textBlue alpha:1.0];
}

/**
 * Restore the app settings to the default settings
 */
- (IBAction)restoreDefaults:(id)sender {
    
    [_defaults setFloat:0.0f forKey:@"toolbarAlpha"];
    [_defaults setFloat:0.5f forKey:@"userScrollSpeed"];
    [_defaults setFloat:0.3f forKey:@"tbRed"];
    [_defaults setFloat:0.3f forKey:@"tbGreen"];
    [_defaults setFloat:0.3f forKey:@"tbBlue"];
    [_defaults setFloat:1.0f forKey:@"textRed"];
    [_defaults setFloat:1.0f forKey:@"textGreen"];
    [_defaults setFloat:1.0f forKey:@"textBlue"];
    [_defaults setFloat:0.0145f forKey:@"userZoomIncrement"];
    [_defaults setFloat:.3f forKey:@"penSize"];
    [_defaults setFloat:.2f forKey:@"eraserSize"];
    [_defaults synchronize];
    
	UIAlertView* alert = [[UILargeAlertView alloc] 
                          initWithText:NSLocalizedString(@"All settings have been reset", nil)
                          fontSize:48];
	[alert show];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if(resetChanges){
        oldDefaults = [_defaults dictionaryRepresentation];
    }
